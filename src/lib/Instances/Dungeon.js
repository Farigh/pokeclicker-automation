...
class AutomationDungeon {
  ...
  static hasShadowPokemons(dungeonName) {
    ...
    return town.region == GameConstants.Region.hoenn && town.subRegion == GameConstants.HoennSubRegions.Orre;
  }
  ...
  static __internal__buildMenu() {
    ...
    const autoDungeonButton = Automation.Menu.addAutomationButton("Auto Fight", this.Settings.FeatureEnabled, autoDungeonTooltip, dungeonDiv, true);
    ...
    const buttonLabel = 'Stop on <span id="automation-dungeon-pokedex-img"><img src="assets/images/pokeball/Pokeball.svg" height="17px"></span> :';
  ...
  static __internal__buildAdvancedSettings(parent) {
    ...
    this.__internal__dungeonBossCatchPokeballSelectElem = Automation.Menu.addPokeballList(this.Settings.BossCatchPokeballToUse, "Pokéball to equip for the boss", bossCatchPokeballTooltip, true);
    ...
    Automation.Menu.addLabeledAdvancedSettingsToggleButton("Skip as many fights as possible", this.Settings.AvoidEncounters, avoidEncountersTooltip, dungeonSettingsPanel);

    // Add the skip boss button
    const skipBossTooltip = "Don't fight the dungeon boss at all." + Automation.Menu.TooltipSeparator + "It will exit the dungeon as soon as the other automation conditions are met";
    Automation.Menu.addLabeledAdvancedSettingsToggleButton("Skip the boss fight", this.Settings.SkipBoss, skipBossTooltip, dungeonSettingsPanel);
  }
  ...
  static __internal__buildChestMinRarityDropdownList(parent) {
    ...
    this.__internal__chestMinRarityDropdownList = Automation.Menu.createDropdownListWithHtmlOptions(selectOptions, "Lowest chest rarity to pick up", tooltip);
    ...
    this.__internal__chestMinRarityDropdownList.onValueChange = function () {
      Automation.Utils.LocalStorage.setValue(this.Settings.SelectedChestMinRarity, this.__internal__chestMinRarityDropdownList.selectedValue);
  ...
  static __internal__toggleCatchStopMode() {
    ...
    this.__internal__currentCatchMode =
      this.__internal__currentCatchMode == this.__internal__CatchModes.UncaughtShadowOrShiny
    ...
    const shadowImageBackground = this.__internal__currentCatchMode.shadow ? `<img src="assets/images/status/shadow.svg" style="${shadowImgStyle} z-index: 1;">` : "";
    const shadowImageForground = this.__internal__currentCatchMode.shadow ? `<img src="assets/images/status/shadow.svg" style="${shadowImgStyle} z-index: 3; opacity: 0.2;">` : "";
  ...
  static __internal__dungeonFightLoop() {
    const forceDungeonProcessing = this.AutomationRequestedMode == this.InternalModes.ForceDungeonCompletion || this.AutomationRequestedMode == this.InternalModes.ForcePokemonFight;

    const avoidFights = Automation.Utils.LocalStorage.getValue(this.Settings.AvoidEncounters) === "true" && this.AutomationRequestedMode != this.InternalModes.ForcePokemonFight;
    const skipChests = this.__internal__chestMinRarityDropdownList.selectedValue == this.__internal__chestTypes.skipall;
    const skipBoss = Automation.Utils.LocalStorage.getValue(this.Settings.SkipBoss) === "true" && !forceDungeonProcessing;
    ...
    if (
      ...
      if (
        ...
        (!forceDungeonProcessing && (this.__internal__playerActionOccured || (Automation.Utils.LocalStorage.getValue(this.Settings.StopOnPokedex) === "true" && this.__internal__isDungeonCompleted())))
      ) {
      ...
      const nonVisibleTiles = this.__internal__isRecovering ? flatBoard.filter((tile) => !tile.isVisited) : flatBoard.filter((tile) => !tile.isVisible);
      ...
      const areAllChestsCollected = discoveredChestsLeftToOpenCount === this.__internal__getChestLeftToOpenCount() || (foundFloorEndTile && !DungeonRunner.map.flash && avoidFights);
      ...
      if (nonVisibleTiles.length === 0 || (areAllBattleDefeated && areAllChestsCollected && (skipBoss || foundFloorEndTile))) {
        if (!avoidFights && visibleEnemiesCount > 0) {
          ...
          if (foundFloorEndTile) {
            ...
            if (ballToCatchBoss != GameConstants.Pokeball.None && this.AutomationRequestedMode == this.InternalModes.None) {
  ...
  static __internal__handleNormalPathing() {
    ...
    const isLastTileOfTheRow = (isEvenRow && playerCurrentPosition.x == maxIndex) || (!isEvenRow && playerCurrentPosition.x == 0);
  ...
  static __internal__handleFlashPathing() {
    ...
    const accessibleUnvisitedTiles = allCells.filter(({ tile, x, y, floor }) => tile.isVisible && !tile.isVisited && DungeonRunner.map.hasAccessToTile({ x, y, floor }));
  ...
  static __internal__markCell(cell) {
    ...
    if (cellType === GameConstants.DungeonTileType.boss || cellType === GameConstants.DungeonTileType.ladder) {
  ...
  static __internal__ensureTheBoardIsInAnAcceptableState() {
    ...
    for (const [rowIndex, row] of DungeonRunner.map.board()[currentFloor].entries()) {
      for (const [columnIndex, tile] of row.entries()) {
        ...
        if (rowIndex === DungeonRunner.map.floorSizes[currentFloor] - 1 && startingTile === null) {
          startingTile = currentLocation;
        }

        if (tile.type() !== GameConstants.DungeonTileType.entrance && this.__internal__isFirstMove) {
    ...
    if (!this.__internal__isFirstMove) {
      ...
      this.__internal__playerActionOccured = this.__internal__playerActionOccured || !DungeonRunner.map.board()[currentFloor].every((row) => row.every((tile) => tile.isVisited));
    }
  ...
  static __internal__updateDivVisibilityAndContent() {
    this.__internal__dungeonFightButton.hidden = !(
      ...
      (App.game.gameState === GameConstants.GameState.town && Automation.Utils.isInstanceOf(player.town, "DungeonTown"))
    );
    ...
    if (!this.__internal__dungeonFightButton.hidden) {
      ...
      if (App.game.gameState !== GameConstants.GameState.dungeon) {
        ...
        if (
          Automation.Utils.LocalStorage.getValue(this.Settings.AvoidEncounters) === "true" &&
  ...
  static __internal__isDungeonCompleted() {
    ...
    if (this.__internal__currentCatchMode.shadow && currentDungeon.allAvailableShadowPokemon().some((p) => App.game.party.getPokemonByName(p)?.shadow < GameConstants.ShadowStatus.Shadow)) {
      return false;
    }
