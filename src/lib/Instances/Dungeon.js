...
class AutomationDungeon {
  ...
  static initialize(initStep) {
    if (initStep == Automation.InitSteps.BuildMenu) {
      ...
      Automation.Utils.LocalStorage.setDefaultValue(
        this.Settings.AvoidEncounters,
        false
      );
      Automation.Utils.LocalStorage.setDefaultValue(
        this.Settings.SkipBoss,
        false
      );

      // Set the default chest rarity to "common" (ie. pickup any chest rarity)
      const formerChestOption = "Dungeon-SkipChests";
      const formerChestOptionValue =
        Automation.Utils.LocalStorage.getValue(formerChestOption);
      if (
        Automation.Utils.LocalStorage.getValue(formerChestOption) != undefined
      ) {
        // TODO Remove this conversion at some point
        const defaultValue =
          formerChestOptionValue == "true"
            ? this.__internal__chestTypes.skipall
            : this.__internal__chestTypes.common;

        // Convert the former setting "Dungeon-SkipChests" to the new chest rarity one
        Automation.Utils.LocalStorage.setDefaultValue(
          this.Settings.SelectedChestMinRarity,
          defaultValue
        );
  ...
  static hasShadowPokemons(dungeonName) {
    ...
    return (
      town.region == GameConstants.Region.hoenn &&
  ...
  static __internal__buildMenu() {
    ...
    const dungeonDiv = Automation.Menu.addCategory(
      "dungeonFightButtons",
      dungeonTitle
    );
    ...
    const autoDungeonTooltip =
      "Automatically enters and completes the dungeon" +
      Automation.Menu.TooltipSeparator +
      "Chests and the boss are ignored until all tiles are revealed\n" +
      "Chests are all picked right before fighting the boss";
    const autoDungeonButton = Automation.Menu.addAutomationButton(
      "Auto Fight",
    ...
    autoDungeonButton.addEventListener(
      "click",
      this.__internal__toggleDungeonFight.bind(this),
      false
    );

    // Add an on/off button to stop after pokedex completion
    const autoStopDungeonTooltip =
      "Automatically disables the dungeon loop\n" +
    ...
    Automation.Menu.addAutomationButton(
      buttonLabel,
      this.Settings.StopOnPokedex,
      autoStopDungeonTooltip,
      dungeonDiv
    );

    // Add the pokéball click action
    this.__internal__pokedexSwitch = document.getElementById(
      "automation-dungeon-pokedex-img"
    );
    this.__internal__pokedexSwitch.onclick =
      this.__internal__toggleCatchStopMode.bind(this);

    // Build advanced settings
    this.__internal__buildAdvancedSettings(
      autoDungeonButton.parentElement.parentElement
    );
  }
  ...
  static __internal__buildAdvancedSettings(parent) {
    ...
    this.__internal__dungeonAdvancedSettingsContainerDivElem =
      dungeonSettingsPanel.parentElement.parentElement;

    // Store the panel button (first child element of the parent div)
    this.__internal__dungeonAdvancedSettingsVisibilityButtonElem =
      dungeonSettingsPanel.parentElement.children.item(0);

    const titleDiv = Automation.Menu.createTitleElement(
      "Dungeon advanced settings"
    );
    titleDiv.style.marginBottom = "10px";
    dungeonSettingsPanel.appendChild(titleDiv);

    // Add the boss catch option
    const bossCatchPokeballTooltip =
    ...
    this.__internal__dungeonBossCatchPokeballSelectElem =
      Automation.Menu.addPokeballList(
        this.Settings.BossCatchPokeballToUse,
        "Pokéball to equip for the boss",
        bossCatchPokeballTooltip,
        true
      );
    dungeonSettingsPanel.appendChild(
      this.__internal__dungeonBossCatchPokeballSelectElem
    );
    this.__internal__dungeonBossCatchPokeballSelectElem.style.marginBottom =
      "3px";
    ...
    const avoidEncountersTooltip =
      "If enabled, it will only fight battles that are not avoidable." +
      Automation.Menu.TooltipSeparator +
    ...
    Automation.Menu.addLabeledAdvancedSettingsToggleButton(
      ...
      this.Settings.AvoidEncounters,
      avoidEncountersTooltip,
      dungeonSettingsPanel
    );

    // Add the skip boss button
    const skipBossTooltip =
      "Don't fight the dungeon boss at all." +
      Automation.Menu.TooltipSeparator +
  ...
  static __internal__buildChestMinRarityDropdownList(parent) {
    const selectOptions = [];
    const savedSelectedRarity = parseInt(
    ...
    for (const rarityName in this.__internal__chestTypes) {
      ...
      selectOptions.push({
        value: chestId,
        element,
        selected: chestId == savedSelectedRarity,
      });
    }

    const tooltip =
      "Choose which chest the automation should consider picking up.\n" +
    ...
    this.__internal__chestMinRarityDropdownList =
      Automation.Menu.createDropdownListWithHtmlOptions(
        selectOptions,
        "Lowest chest rarity to pick up",
        tooltip
      );
    this.__internal__chestMinRarityDropdownList.getElementsByTagName(
      "button"
    )[0].style.width = "140px";
  ...
  static __internal__toggleCatchStopMode() {
    // Switch mode
    this.__internal__currentCatchMode =
      this.__internal__currentCatchMode ==
      this.__internal__CatchModes.UncaughtShadowOrShiny
        ? this.__internal__CatchModes.Uncaught
        : Object.entries(this.__internal__CatchModes).find(
            (x) => x[1].id == this.__internal__currentCatchMode.id + 1
          )[1];

    // Update the image accordingly
    const image = this.__internal__currentCatchMode.shiny
      ? "Pokeball-shiny"
      : "Pokeball";

    const shadowImgStyle =
      "position: absolute; right: -4px; bottom: 1px; height: 25px;";
    ...
    this.__internal__pokedexSwitch.innerHTML = `${shadowImageBackground}<img src="assets/images/pokeball/${image}.svg" ${pokeballImgStyle}>${shadowImageForground}`;
  }
  ...
  static __internal__toggleDungeonFight(enable) {
    ...
    if (enable) {
      ...
      if (this.__internal__autoDungeonLoop === null) {
        ...
        this.__internal__autoDungeonLoop = setInterval(
          this.__internal__dungeonFightLoop.bind(this),
          50
  ...
  static __internal__dungeonFightLoop() {
    const forceDungeonProcessing =
      this.AutomationRequestedMode ==
        this.InternalModes.ForceDungeonCompletion ||
      this.AutomationRequestedMode == this.InternalModes.ForcePokemonFight;

    const avoidFights =
      Automation.Utils.LocalStorage.getValue(this.Settings.AvoidEncounters) ===
        "true" &&
      this.AutomationRequestedMode != this.InternalModes.ForcePokemonFight;
    const skipChests =
      this.__internal__chestMinRarityDropdownList.selectedValue ==
      this.__internal__chestTypes.skipall;
    const skipBoss =
      Automation.Utils.LocalStorage.getValue(this.Settings.SkipBoss) ===
        "true" && !forceDungeonProcessing;
    ...
    if (
      ...
      if (
        this.__internal__stopAfterThisRun ||
      ...
      const flatBoard = DungeonRunner.map
        .board()
        [DungeonRunner.map.playerPosition().floor].flat();
      // If recovering, only end if all tiles are visited, otherwise, when all cells are visible
      const nonVisibleTiles = this.__internal__isRecovering
        ? flatBoard.filter((tile) => !tile.isVisited)
        : flatBoard.filter((tile) => !tile.isVisible);
      const visibleEnemiesCount = flatBoard.filter(
        (tile) =>
          tile.isVisible && tile.type() === GameConstants.DungeonTileType.enemy
      ).length;
      const discoveredChestsLeftToOpenCount =
        this.__internal__chestPositions.length;
      ...
      const areAllBattleDefeated =
        avoidFights ||
        visibleEnemiesCount ===
          DungeonRunner.map.totalFights() - DungeonRunner.encountersWon();
      // Either we are skipping chests, or all remaining chests are visible
      const areAllChestsCollected =
        discoveredChestsLeftToOpenCount ===
          this.__internal__getChestLeftToOpenCount() ||
      ...
      if (
        nonVisibleTiles.length === 0 ||
        (areAllBattleDefeated &&
          areAllChestsCollected &&
          (skipBoss || foundFloorEndTile))
      ) {
        if (!avoidFights && visibleEnemiesCount > 0) {
          ...
          DungeonRunner.map.floorSizes.length - 1
        ) {
          ...
          if (foundFloorEndTile) {
            ...
            const ballToCatchBoss = parseInt(
              Automation.Utils.LocalStorage.getValue(
                this.Settings.BossCatchPokeballToUse
              )
            );
            if (
              ballToCatchBoss != GameConstants.Pokeball.None &&
              this.AutomationRequestedMode == this.InternalModes.None
            ) {
  ...
  static __internal__handleNormalPathing() {
    ...
    const maxIndex =
      DungeonRunner.map.board()[playerCurrentPosition.floor].length - 1;
    const isEvenRow = (maxIndex - playerCurrentPosition.y) % 2 == 0;
    const isLastTileOfTheRow =
      (isEvenRow && playerCurrentPosition.x == maxIndex) ||
    ...
    if (
      playerCurrentPosition.y == maxIndex &&
      playerCurrentPosition.x != 0 &&
      !DungeonRunner.map.board()[playerCurrentPosition.floor][
        playerCurrentPosition.y
      ][playerCurrentPosition.x - 1].isVisited
    ) {
    ...
    this.__internal__markCell({
      ...point,
      tile: DungeonRunner.map.board()[point.floor][point.y][point.x],
    });
  }
  ...
  static __internal__handleFlashPathing() {
    ...
    const allCells = currentBoard.flatMap((row, y) =>
      row.map((tile, x) => ({ tile, x, y, floor }))
    );
    ...
    const nonEnemyCells = accessibleUnvisitedTiles.filter(
      ({ tile }) => tile.type() !== GameConstants.DungeonTileType.enemy
    );
    const enemyCells = accessibleUnvisitedTiles.filter(
      ({ tile }) => tile.type() === GameConstants.DungeonTileType.enemy
    );
    if (nonEnemyCells.length > 0) {
      const bestEmptyCell =
        this.__internal__getCellWithMostNonVisitedNeightbours(nonEnemyCells);
    ...
    if (enemyCells.length > 0) {
      const bestEnemyCell =
        this.__internal__getCellWithMostNonVisitedNeightbours(enemyCells);
  ...
  static __internal__getCellWithMostNonVisitedNeightbours(cellsToChooseFrom) {
    ...
    for (const cell of cellsToChooseFrom) {
      const nonVisitedNeighborsCount =
        this.__internal__getNonVisitedNeighboursCount(cell);
      if (
        nonVisitedNeighborsCount >=
        cellWithMostNonVisitedNeighbors.nonVisitedNeighborsCount
      ) {
        cellWithMostNonVisitedNeighbors = {
          ...cell,
          nonVisitedNeighborsCount: nonVisitedNeighborsCount,
        };
      }
    }
  ...
  static __internal__markCell(cell) {
    ...
    if (
      cellType === GameConstants.DungeonTileType.boss ||
  ...
  static __internal__ensureTheBoardIsInAnAcceptableState() {
    ...
    for (const [rowIndex, row] of DungeonRunner.map
      .board()
      ...
      for (const [columnIndex, tile] of row.entries()) {
        ...
        if (
          rowIndex === DungeonRunner.map.floorSizes[currentFloor] - 1 &&
          startingTile === null
        ) {
        ...
        if (
          tile.type() !== GameConstants.DungeonTileType.entrance &&
    ...
    if (!this.__internal__isFirstMove) {
      ...
      this.__internal__playerActionOccured =
        ...
        !DungeonRunner.map
          .board()
          [currentFloor].every((row) => row.every((tile) => tile.isVisited));
    }
    ...
    if (
      !this.__internal__floorEndPosition &&
      this.__internal__playerActionOccured
    ) {
  ...
  static __internal__updateDivVisibilityAndContent() {
    this.__internal__dungeonFightButton.hidden = !(
      ...
      if (App.game.gameState !== GameConstants.GameState.dungeon) {
        ...
        if (
          this.AutomationRequestedMode !=
            this.InternalModes.ForceDungeonCompletion &&
          this.AutomationRequestedMode !=
            this.InternalModes.ForcePokemonFight &&
          Automation.Utils.LocalStorage.getValue(this.Settings.StopOnPokedex) ==
            "true" &&
          this.__internal__isDungeonCompleted()
        ) {
          disableNeeded = true;
    ...
    // If the panel is hidden and the settings panel is opened, close the later to restore all the other settings visibility
      ...
      this.__internal__dungeonAdvancedSettingsContainerDivElem.hasAttribute(
        "automation-visible"
      )
    ) {
  ...
  static __internal__isDungeonCompleted() {
    ...
    if (
      this.__internal__currentCatchMode.shadow &&
      currentDungeon
    ...
    return DungeonRunner.dungeonCompleted(
      currentDungeon,
      this.__internal__currentCatchMode.shiny
    );
  }
  ...
  static __internal__addChestPosition(cell) {
    ...
    const currentChestRarity =
      this.__internal__chestTypes[cell.tile.metadata.tier];
    if (
      currentChestRarity <
      this.__internal__chestMinRarityDropdownList.selectedValue
    ) {
    ...
    if (
      !this.__internal__chestPositions.some(
        (pos) => pos.x == cell.x && pos.y == cell.y && pos.floor == cell.floor
      )
    ) {
  ...
  static __internal__getChestLeftToOpenCount() {
    ...
    for (const tile of DungeonRunner.map.board().flat().flat()) {
      if (tile.type() == GameConstants.DungeonTileType.chest) {
        const currentChestRarity =
          Automation.Dungeon.__internal__chestTypes[tile.metadata.tier];
