...
class AutomationFocusAchievements {
  ...
  static __internal__advancedSettings = {
    ...
    AchievementEnabled: function (type, amount) {
      return `Focus-Achievements-${type}-${amount}-Enabled`;
    }.bind(this),
  };
  ...
  static __registerFunctionalities(functionalitiesList) {
    functionalitiesList.push({
      ...
      name: "Achievements",
      tooltip:
        "Completes the pending achivements" +
  ...
  static __internal__buildAchievementSelectionSettings(parent) {
    // Add the description
    const tooltip =
      "Enabling a quest of a given type will automatically enable\n" +
    ...
    for (const [index, achievementData] of achievementDataList.entries()) {
      ...
      achievementData.toggleButton.addEventListener(
        "click",
        function () {
          // Don't do anything if the feature is disabled
          if (this.__internal__currentAchievement) {
  ...
  static __internal__getAchievementsData() {
    ...
    for (const achievement of this.__internal__filteredAchievementList) {
      const data = {
        type: achievement.property.constructor.name,
        amount: achievement.property.requiredValue,
      };
  ...
  static __internal__getAchievementLabel(achievementData) {
    const label =
      achievementData.type == "RouteKillRequirement"
        ? `Defeat ${achievementData.amount} Pokémon on <Route>`
        : achievementData.type == "ClearGymRequirement"
        ? `Defeat <Gym> in <Region> ${achievementData.amount} times`
