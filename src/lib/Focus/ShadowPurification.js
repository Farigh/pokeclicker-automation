...
class AutomationFocusShadowPurification {
  ...
  static __registerFunctionalities(functionalitiesList) {
    ...
    const isUnlockedCallback = function () {
      return pokeballFilterOptions.shadow.canUse();
    };
    ...
    functionalitiesList.push({
      ...
      name: "Shadow purify",
      tooltip:
        "Hunts for pokémons that are corrupted by shadow" +
  ...
  static __internal__start() {
    ...
    this.__internal__shadowPurificationLoop = setInterval(
      this.__internal__focusOnShadowPurification.bind(this),
      10000
  ...
  static __internal__focusOnShadowPurification() {
    ...
    if (Automation.Utils.isInInstanceState()) {
      if (
        this.__internal__currentDungeonData == null ||
        !this.__internal__doesDungeonHaveAnyUncaughtShadowPokemon(
    ...
    // No more dungeons, focus on purification
      ...
      App.game.party.caughtPokemon.some(
      ...
      Automation.Notifications.sendWarningNotif(
        "No more available Shadow pokémons to capture or Purify.\nTurning the feature off",
        "Focus"
      );
    }
  }
  ...
  static __internal__captureShadowPokemons() {
    ...
    Automation.Dungeon.setBeforeNewRunCallBack(
      this.__internal__tryToPurifyShadowPokemon.bind(this)
    );
  ...
  static __internal__setNextShadowDungeon() {
    ...
    if (
      ...
      const index = this.__internal__shadowDungeonData.indexOf(
    ...
    this.__internal__currentDungeonData =
      this.__internal__shadowDungeonData.find(
        (data) =>
          this.__internal__doesDungeonHaveAnyUncaughtShadowPokemon(
            data.dungeon,
            true
   ...
   */
  static __internal__doesDungeonHaveAnyUncaughtShadowPokemon(
    dungeon,
    onlyConsiderAvailablePokemons = false
  ) {
  ...
  static __internal__getEveryPokemonForDungeon(
    ...
    for (const enemy of enemyList) {
      ...
      if (onlyConsiderAvailablePokemons) {
        const isEnemyLocked = enemy.options?.requirement
          ? !enemy.options?.requirement.isCompleted()
          : false;
